...
def reconstructed_image(D, c, num_coeffs, X_mean, n_blocks, im_num):
    ...
    #    '''
    sz = 256 / n_blocks
    c_im = c[:num_coeffs, n_blocks * n_blocks *
             im_num:n_blocks * n_blocks * (im_num + 1)]
    D_im = D[:, :num_coeffs]
    recon_data = np.zeros((256, 256), dtype=np.float32)
    for i in range(256 / sz):
        for j in range(256 / sz):
            block = np.dot(
                D_im, c_im[:, i * (256 / sz) + j]).reshape(sz, sz) + X_mean
...
def plot_reconstructions(D, c, num_coeff_array, X_mean, n_blocks, im_num):
    ...
    for i in range(3):
        for j in range(3):
            plt.axes(axarr[i, j])
            plt.imshow(reconstructed_image(D, c, num_coeff_array[
                       i * 3 + j], X_mean, n_blocks, im_num))
...
def plot_top_16(D, sz, imname):
    ...
    for i in range(4):
        for j in range(4):
            ...
            D_comp = 255 * (D_comp - np.min(D_comp)) / \
                (np.max(D_comp) - np.min(D_comp))
        ...
        for i in range(0, no_images):
            for j in range(0, Ims.shape[1] / sz):
                for k in range(0, Ims.shape[2] / sz):
                    temp = Ims[
                        i, sz * j:sz * (j + 1), sz * k:sz * (k + 1)].reshape((1, sz * sz))
                    X[index, :] = temp
                    index = index + 1
